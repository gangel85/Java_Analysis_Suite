...
public class CutsMap {
	...
	/* --- Constructors
	   ---
	 */
	CutsMap(ReadParameters parameters){
		
		// Generating the  standard cuts 
		EC_Cut FC_PCAL_el = new EC_Cut(parameters);
		FC_PCAL_el.setName("PCAL_el");
		EC_Cut FC_EC1_el = new EC_Cut(parameters);
		
		DC_Cut FC_DC1_el = new DC_Cut(parameters);
		FC_DC1_el.setName("DC1_el");
	
		Sampling_Fractions SF_el = new Sampling_Fractions(parameters); 
		SF_el.setName("SamplingFraction_el");
		
		DC_Cut FC_DC3_el = new DC_Cut(parameters);
		FC_DC3_el.setName("DC3_el");
		EC_Cut FC_PCAL_ph = new EC_Cut(parameters);
		FC_PCAL_ph.setName("PCAL_ph");
		EC_Cut FC_EC1_ph = new EC_Cut(parameters);
		FC_EC1_ph.setName("EC1_ph");
		EC_Cut FC_EC2_ph = new EC_Cut(parameters);
	...
	//	List<DetectorCut> PhotonCuts = new ArrayList<DetectorCut>();
		...
		// Cuts for PiP
		DC_Cut FC_DC_pip = new DC_Cut(parameters);
		...
		//Cuts for PiM
		DC_Cut FC_DC_pim = new DC_Cut(parameters);
