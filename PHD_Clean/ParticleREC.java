...
public class ParticleREC extends Particle {
	...
	Map<Integer, Vector3> ParticleTrackNDF = new HashMap<Integer, Vector3>();
...
//	int DC_layer1 =6 , DC_layer2=18, DC_layer3=30 ;  
	...
	// Constructor using the index in REC banks 
	public ParticleREC(int index,Bank RPart,Bank Rec_Traj,Bank Rec_Calorimeter ) {
		...
		float PionMomentumX=RPart.getFloat("px", index) ;
		float PionMomentumY=RPart.getFloat("py", index) ; 
		float PionMomentumZ=RPart.getFloat("pz", index) ; 
		float Verte_X =RPart.getFloat("vx", index) ;
		float Verte_Y =RPart.getFloat("vy", index) ;
		float Verte_Z =RPart.getFloat("vz", index) ;
		int Particle_PID =RPart.getInt("pid",index) ; 
		int Particle_Charge =(int)RPart.getByte("charge",index) ;  

	
		Vector3 myvector = new Vector3(PionMomentumX,PionMomentumY,PionMomentumZ );
		
		double momentum = Math.sqrt(PionMomentumX*PionMomentumX+PionMomentumY*PionMomentumY+PionMomentumZ*PionMomentumZ);
		...
		this.vZ=Verte_Z;
		this.mom=momentum;
		this.Theta=myvector.theta();
		this.Phi=myvector.phi();
		int pid = Particle_PID;		
		int charge = Particle_Charge;
		
		// Looking over the other variables 
		...
		Map<Integer,List<Integer>> trajMap = loadMapByIndex(Rec_Traj,"pindex");
			...
			this.index = index;
			if(caloMap.get(index)!=null){  
				CalCheck=true;
				//Am I sure about this? 
				for (int icalo : caloMap.get(index)) {
			...
			if(charge!=0 && trajMap.get(index)!=null ){
				//System.out.println(trajMap.get(index).size());
				for(int iin =0 ; iin<trajMap.get(index).size(); iin ++ ) {
					DCCheck=true;
					if( Rec_Traj.getInt("detector",trajMap.get(index).get(iin)) == DC_detector && Rec_Traj.getInt("layer",trajMap.get(index).get(iin))== DC_layer1){
						dx = Rec_Traj.getFloat("x",trajMap.get(index).get(iin));
						dy = Rec_Traj.getFloat("y",trajMap.get(index).get(iin));
						dz = Rec_Traj.getFloat("z",trajMap.get(index).get(iin));
					//	System.out.println(" My Pindex is " + Rec_Traj.getInt("pindex",trajMap.get(index).get(iin)) + " dx "+dx );
					...
					else if( Rec_Traj.getInt("detector",trajMap.get(index).get(iin)) == DC_detector && Rec_Traj.getInt("layer",trajMap.get(index).get(iin))== DC_layer2){
					...
					else if( Rec_Traj.getInt("detector",trajMap.get(index).get(iin)) == DC_detector && Rec_Traj.getInt("layer",trajMap.get(index).get(iin))== DC_layer3){
				...
				//Rec_Traj.show();
				System.out.println("cavolo! ho le banche DC am non ho hit??");
				System.out.println(" Charge is + " + charge );
				System.out.println( " Traj Map " + trajMap.get(index));
	...
	// Constructor using the class Particle
	public ParticleREC(Particle particle,Bank Rec_Part,Bank Rec_Traj,Bank Rec_Calorimeter) {
		...
		for (int ipart =0 ; ipart<Rec_Part.getRows(); ipart++) {
			double p_pi = Math.sqrt(Rec_Part.getFloat("px", ipart)*Rec_Part.getFloat("px", ipart)+Rec_Part.getFloat("py", ipart)*Rec_Part.getFloat("py", ipart)+Rec_Part.getFloat("pz", ipart)*Rec_Part.getFloat("pz", ipart));
			double p_pi_x=Rec_Part.getFloat("px", ipart);
			double p_pi_y= Rec_Part.getFloat("py", ipart);
			
			if(pid == Rec_Part.getInt("pid", ipart) && particle.p() <= p_pi+0.1 && particle.p() >= p_pi-0.1 &&  particle.px() <= p_pi_x+0.1 && particle.px() >= p_pi_x-0.1 &&  particle.py() <= p_pi_y+0.1 && particle.py() >= p_pi_y-0.1) {
				//System.out.println(Rec_Part.getInt("pid", ipart));
				this.index = ipart;
 ...
 public  int getSector()
 {
	...
	Math.pow(Hit.z(), 2)), Hit.x() / Math.sqrt(Math.pow(Hit.x(), 2)
	   ...
	   return 0;
		...
		public double el_phiDC2() {
			  double phi = 180 / Math.PI* Math.atan2(elVectorDC2.y() /
						Math.sqrt(Math.pow(elVectorDC2.x(), 2) + Math.pow(elVectorDC2.y(), 2) +
						Math.pow(elVectorDC2.z(), 2)), elVectorDC2.x() / Math.sqrt(Math.pow(elVectorDC2.x(), 2)
						                + Math.pow(elVectorDC2.y(), 2) + Math.pow(elVectorDC2.z(), 2)));
			// TODO Auto-generated method stub
			return phi ;
		}
