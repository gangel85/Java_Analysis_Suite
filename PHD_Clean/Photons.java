...
public class Photons {
	...
	public Photons(ParticleREC electron, Bank Rec_Part,  Bank Rec_Calorimeter)
	{  double vectAngles = 0; boolean EC_cut = false;
		...
		if(pid == 22){
			...
			/*
				System.out.println("First detector " + First_det);
				if (First_det!=1) { System.out.println(" ---> Attention this photon starts in EC2");
				for(int q=0 ; q< caloMap.get(ipart).size(); q++) {
					System.out.println(" Entry" +q + " Layer " + Rec_Calorimeter.getInt("layer", caloMap.get(ipart).get(q)));
				}
				System.out.println(" ---------- ");
				}
				*/
