...
public class Sampling_Fractions implements DetectorCut{
	...
	private double el_Min_En;
	
// TO ADD sector by sector 
	Sampling_Fractions(ReadParameters parameters){
		Sigma=parameters.Get_el_SFSigma();	
		el_Min_En=parameters.Get_el_MinEn();
	}
...
//MC		
		...
		// if(particle.p()>7) {
		  mean= 2.48605e-1-8.44221e-01/1000*Math.pow(particle.p()-4.8777,2);
		  sigma= 7.41575e-3+2.15861e-1/(10*(particle.p()*3.19801e-1));
		  upper_lim_total = mean + sigma_range * sigma;
	      lower_lim_total = mean - sigma_range * sigma;
		
		  //System.out.println(" Sigma "+sigma+ " MEAN  "+mean);
		// }
		  ...
		  pass_triangle=true; // FOr now I remove that cut 
		  if(pass_band && pass_triangle && Epcal>el_Min_En) return true;
  ...
  if(sect -1  == k){
    ...
    lower_lim_total = mean - sigma_range * sigma;
		...
		if(particle.p() < 4.5){ pass_triangle = true;}
		 ...
		 if(pass_band &&  Epcal> el_Min_En && (ratioPcal+ratioEin+ratioEout)>0.2  ) {
